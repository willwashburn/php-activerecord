<?php
	...
	function ar_get_relationships($array, $keys = 'all')
	{
		$return = array();

		if ($keys == 'all') {
			foreach ($array['__relationships'] as $key=> $value) {
				$return[$key] = $value['attributes'];
			}
		} else {
			if (is_array($keys)) {
				foreach ($keys as $value) {
					$return[$value] = $array['__relationships'][$value]['attributes'];
				}
			} else {
				$return = $array['__relationships'][$keys]['attributes'];
			}
