<?php
    ...
    class Model
    {
         ...
         * then
         *
         * @param \ActiveRecord\will|array $attributes             Hash containing names and values to mass assign to the model
         ...
         *
         * @internal param \ActiveRecord\will $attributes be mapped according to the schema's defaults. Otherwise, the given
         ...
         * @author  Will
         */
        static public function insert_many($values)
        {
            $table   = static::table();
        ...
        /*
        * Alias for first() with conditions order by RAND() limit 1
        * @oa Will
        */
        public static function random( /* ... */)
        {

            return call_user_func_array('static::find', array_merge(array('random'), func_get_args()));
        }
        ...
        public static function find( /* $type, $options */)
        {
            ...
            if ($num_args > 0 && ($args[0] === 'all' || $args[0] === 'first' || $args[0] === 'last' || $args[0] === 'random')) {
                switch ($args[0]) {
                    case 'all':
                        $single = FALSE;
                        break;
                    ...
                    case 'random':
                        if (!isset($options['limit'])) {
                            $options['limit']  = 1;
                            $options['offset'] = 0;
                        } elseif ($options['limit'] > 1) {
                            $single = FALSE;
                        }
                        $options['order'] = 'RAND() DESC';
...
// Get the name of the current connection
        ...
        */
        ...
        public function panelize($function_name = FALSE, $unset = array())
        {
            ...
            $return = \commonlib\object_converter::toArray($this);
            if ($function_name) {
                $this->$function_name($return, $unset);
            }
