<?php
    ...
    class SQLBuilder
    {
        ...
        public function insert_many($hash, $sequence_name = NULL)
        {

//            if (!is_hash($hash))
            //              throw new ActiveRecordException('Inserting requires a hash.');

            $this->operation = 'INSERT_MANY';
            $this->data      = $hash;
         ...
         * @param array  $values
         * @param        $map  A hash of "mapped_column_name" => "real_column_name"
         * @internal param Array $args of values for each attribute in $name
        ...
        private function build_insert_many()
        {
            $keys = join(',', $this->quoted_multi_key_names());

            $sql = "INSERT INTO $this->table($keys) VALUES ";

            $number_of_keys           = count($this->quoted_multi_key_names());
            $number_of_rows_to_insert = count($this->data);
            ...
            while ($number_of_rows_to_insert > 0) {
                $sql .= $values_string;
        ...
        private function quoted_multi_key_names()
        {

            if (is_object($this->data[0])) {
                $data_set = (array)$this->data[0];
            } else {
                $data_set = $this->data[0];
            }
